...
btnEncrypt.addEventListener('click', function () {
    ...
    if (checkbox1 && image_output && checkbox1.checked) {
        downloadBlob(image_output.encryptedImage1, `encrypted1-${image_output.name}`);
    }
    if (checkbox2 && image_output && checkbox2.checked) {
        downloadBlob(image_output.encryptedImage2, `encrypted2-${image_output.name}`);
    }
    if (checkbox3 && image_output && checkbox3.checked) {
        downloadBlob(image_output.encryptedImage3, `encrypted3-${image_output.name}`);
    }
});
