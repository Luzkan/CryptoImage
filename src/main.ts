...
function loadResultImagesToResultSection(image_output: any){
  ...
  loadImage(image_output.singValDecompBMPEncrypted.toBlob(), function (img: any) { markResultImage(img, singValDecompEncodedFullscreenDiv); singValDecompEncodedDiv.appendChild(img); }, { maxWidth: 300 });
  ...
  loadImage(image_output.singValDecompBMPDecrypted.toBlob(), function (img: any) { markResultImage(img, singValDecompDecodedFullscreenDiv); singValDecompDecodedDiv.appendChild(img); }, { maxWidth: 300 });
}
...
function loadResultImagesToFullscreenContainers(image_output: any){
  ...
  loadImage(image_output.singValDecompBMPEncrypted.toBlob(), function (img: any) { markResultFullScreenImage(img); singValDecompEncodedFullscreenDiv.appendChild(img); }, {  });
  ...
  loadImage(image_output.singValDecompBMPDecrypted.toBlob(), function (img: any) { markResultFullScreenImage(img); singValDecompDecodedFullscreenDiv.appendChild(img); }, {  });
}
